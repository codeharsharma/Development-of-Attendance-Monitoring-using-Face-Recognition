...
class Register:
 ...
 #bg image
        ...
        loginbtn=Button(frame,command=self.log,text="Log in", font=("times new roman",15,"bold"),bd=0,relief=RIDGE,fg="#fff",bg="#002B53",activeforeground="white",activebackground="#007ACC")
    ...
    def reg(self):
        if (self.var_fname.get()=="" or self.var_lname.get()=="" or self.var_cnum.get()=="" or self.var_email.get()=="" or self.var_ssq.get()=="Select" or self.var_sa.get()=="" or self.var_pwd.get()=="" or self.var_cpwd.get()==""):
            ...
            messagebox.showerror("Error","Please enter the same password and confirm password!",parent=self.root)
        elif(self.var_check.get()==0):
            messagebox.showerror("Error","Please check the Terms and Conditions that have been agreed!",parent=self.root)
        else:
            ...
            try:
                ...
                if row!=None:
                    messagebox.showerror("Error","User already exists, please try a different Email",parent=self.root)
                else:
